...
export class RefreshGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
      ...
      const token = this.extractTokenFromCookie(request) || this.extractTokenFromHeader(request) || this.extractTokenFromBody(request);
      ...
      try {
        ...
        if(!user.sessions.some(session => session.refreshToken === payload.sub)) {
          throw new UnauthorizedException(unauthMessage);
        }
